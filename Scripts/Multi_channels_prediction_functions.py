...
def load_data(channels, drug_dir, cancer_dir, response_dir):
    #Response data
    res = pd.read_csv(response_dir, sep='\t', engine='c',
                      na_values=['na', '-', ''], header=0, index_col=None)

    # Drug data
    files = os.listdir(drug_dir)
    image = np.empty((len(files), 40, 40, 1))
    sample = []
    id = []
    for i in range(len(files)):
        if files[i].split('.')[1] == 'txt' and files[i].split('_')[1] == 'Drug':
            id.append(i)
            data = pd.read_csv(drug_dir + files[i], sep='\t', engine='c',
                               na_values=['na', '-', ''], header=None, index_col=None)
            image[i, :, :, 0] = data.values
            sample.append(files[i].split('_data.txt')[0][1:])
    image = image[id, :, :, :]
    drug = {}
    drug['data'] = image
    drug['sample'] = sample
    ...
    # Cancer data
    files = os.listdir(cancer_dir)
    image = np.empty((len(files), 45, 45, channels))
    sample = []
    id = []
    cnt = 0
    for i in range(len(files)):
        if files[i].split('.')[1] == 'txt' and files[i].split('_')[1].split('-')[0] == 'ACH':
            if int(files[i].split('.txt')[0][-1]) >= channels: # To control number of channels
                continue
            if files[i].split('_')[1] not in sample:
                sample.append(files[i].split('_')[1])
                id.append(cnt)
                cnt+=1
                data = pd.read_csv(cancer_dir + files[i], sep='\t', engine='c',
                               na_values=['na', '-', ''], header=None, index_col=None)
                image[cnt, :, :, int(files[i].split('.txt')[0][-1])] = data.values
            else:
                id1 = sample.index(files[i].split('_')[1])
                data = pd.read_csv(cancer_dir + files[i], sep='\t', engine='c',
                               na_values=['na', '-', ''], header=None, index_col=None)
                image[id1, :, :, int(files[i].split('.txt')[0][-1])] = data.values
...
def get_data_for_cross_validation_drug_spec(res, ccl, sampleID):

    trainData = []
    valData = []
    testData = []
    
    train_idd = ID_mapping(ccl['sample'], res.iloc[sampleID['trainID'], :].improve_sample_id)
    trainData.append(ccl['data'][train_idd, :, :, :])
    val_idd = ID_mapping(ccl['sample'], res.iloc[sampleID['valID'], :].improve_sample_id)
    valData.append(ccl['data'][val_idd, :, :, :])
    test_idd = ID_mapping(ccl['sample'], res.iloc[sampleID['testID'], :].improve_sample_id)
    ...
    train['sample'] = res.iloc[sampleID['trainID'], :].improve_sample_id + '|' + res.iloc[sampleID['trainID'], :].improve_chem_id
    val = {}
    val['data'] = valData
    val['label'] = valLabel
    val['sample'] = res.iloc[sampleID['valID'], :].improve_sample_id + '|' + res.iloc[sampleID['valID'], :].improve_chem_id
    test = {}
    test['data'] = testData
    test['label'] = testLabel
    test['sample'] = res.iloc[sampleID['testID'], :].improve_sample_id + '|' + res.iloc[sampleID['testID'], :].improve_chem_id
...
def get_data_for_cross_validation(res, ccl, drug, sampleID):
    ...
    train_idd = ID_mapping(drug['sample'], res.iloc[sampleID['trainID'], :].improve_chem_id)
    trainData.append(drug['data'][train_idd, :, :, :])
    val_idd = ID_mapping(drug['sample'], res.iloc[sampleID['valID'], :].improve_chem_id)
    valData.append(drug['data'][val_idd, :, :, :])
    test_idd = ID_mapping(drug['sample'], res.iloc[sampleID['testID'], :].improve_chem_id)
...
class CNN2D_Regressor():
    ...
    def __init__(self, params, input_data_dim, dropout):
        ...
        for input_id in range(num_input):
            in_id = Input(shape=(self.input_data_dim[input_id][0], self.input_data_dim[input_id][1], self.input_data_dim[input_id][2]),
...
def CNN2D_Regression_Analysis(train, resultFolder, para, val=None, test=None, batch_size=None):
    '''
    ...
    if isinstance(trainData, list):
        ...
        for i in range(len(trainData)):
            input_data_dim.append([trainData[i].shape[1], trainData[i].shape[2], trainData[i].shape[3]])
    else:
        input_data_dim = [[trainData.shape[1], trainData.shape[2], trainData.shape[3]]]
